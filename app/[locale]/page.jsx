...
export async function generateSchemas(t, locale) {
  ...
  const schema_url =
    locale && locale !== "default" ? `${baseUrl}/${locale}` : baseUrl;
  return {
  ...
  return (
    ...
    >
      ...
      <main className="flex flex-col items-center justify-center w-full">
        <section className="min-h-[calc(100svh+20px)] hero flex items-center justify-center w-full bg-hero-light bg-hero-dark">
          <div className="hero-overlay bg-opacity-60"></div>
          <div className="hero-content text-center text-neutral-content">
            <div className="max-w-lg sm:p-0 p-12">
              <p className="text-5xl font-bold">🌍</p>
              <h1 className="mb-5 text-5xl font-bold">{t("hero_title")}</h1>
              <h2 className="mb-5 text-xl">{t("hero_subtitle")}</h2>
        ...
        <section
          id="about"
          className="flex flex-col gap-12 w-full items-start md:items-center justify-center p-8"
        >
          <h2
            className="text-7xl font-bold text-center mx-auto underline"
            data-aos="fade-up"
          >
            {t("about_title")}
          </h2>
          ...
          <HomeArticle
            ...
            image={{
              ...
              alternate:
                "Mapa de Black Maps llamado: Club de ESTADOS UNIDOS 🇺🇸 más buscado en Google por país (desde 2004)",
            }}
            locale={locale}
            alignement="right"
            aos="fade-up"
